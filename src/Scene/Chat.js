...
class Chat extends Component {
  ...
  Authorized(){
    this.socket.on('authorized', res => {
      ...
      if(res.ok){
        if(res.token != undefined) store.dispatch({type:"PATCH_TOKEN",payload: res.token });
      }else{
        ...
        for(key in res.data.reverse()){
          ...
          let msgObj = {
            ...
            user: {
              ...
              name: (data.sender.role.id == 3) ? this.props.AuthUserReducer.info.fullName : data.request.carrier.name,
              avatar: (data.sender.role.id == 3) ? API_URL.Host + this.props.AuthUserReducer.info.imagePath : ''
            },
  ...
  render() {
    let { ChatReducer: { messages } } = this.props;
    return (
      <View  style={{ flex: 1 }}>
